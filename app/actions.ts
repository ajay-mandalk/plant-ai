...
export async function suggestQuestions(history: any[]) {
  ...
  const { object } = await generateObject({
    model: agrofly.languageModel('agrofly-g2'),
    temperature: 0,
    maxTokens: 512,
...
const groupInstructions = {
  web: `
  You are an AI web search engine called agrofly, designed to help users find information on the internet with no unnecessary chatter and more focus on the content and responsed with markdown format and the response guidelines below.
  ...
  chat: `
  You are agrofly, a helpful assistant that helps with the task asked by the user.
  Today's date is ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: '2-digit', weekday: 'short' })}.
