...
import { agrofly } from '@/ai/providers';
  ...
  if (!globalStreamContext) {
    try {
      if (!process.env.REDIS_URL || !process.env.REDIS_URL.startsWith('redis://')) {
        console.warn('Invalid Redis URL - streams will not be resumable');
        return null;
      }
...
export async function POST(req: Request) {
  ...
  const authRequiredModels = ['agrofly-anthropic', 'agrofly-google'];
  ...
  if (user) {
    ...
    const freeUnlimitedModels = ['agrofly-default', 'agrofly-vision'];
  ...
  const stream = createDataStream({
    execute: async (dataStream) => {
      const result = streamText({
        ...
        model: agrofly.languageModel(model),
        messages: convertToCoreMessages(messages),
        maxTokens: 8192,
        ...(model.includes('agrofly-qwq') || model.includes('agrofly-qwen-32b')
          ? {
            temperature: 0.6,
            topP: 0.95,
          }
        ...
        providerOptions: {
          openai: {
            ...(model === 'agrofly-o4-mini' || model === 'agrofly-o3'
              ? {
          ...
          anthropic: {
            ...(model === 'agrofly-anthropic-thinking' || model === 'agrofly-opus-pro'
              ? {
                    ...
                    if (!result.title || result.title.trim() === '') {
                      try {
                        const { object } = await generateObject({
                          model: agrofly.languageModel('agrofly-nano'),
...
plt.show()`;
              ...
              const sandbox = await daytona.create(
                {
                  snapshot: 'agrofly-analysis:1751171803',
                }
              );
...
`;
        ...
        experimental_repairToolCall: async ({ toolCall, tools, parameterSchema, error }) => {
          ...
          const { object: repairedArgs } = await generateObject({
            model: agrofly.languageModel('agrofly-default'),
        ...
        onFinish: async (event) => {
          ...
          console.log('Sources: ', event.sources);
           ...
           // Implement fallback logic or retry with a different model if the response body is empty
            if (!event.response.body) {
           console.error('Empty response body from model');
