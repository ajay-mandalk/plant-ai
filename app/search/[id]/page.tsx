...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  // if not chat, return agrofly Chat
  if (!chat) {
    return { title: 'agrofly Chat' };
  }
  let title;
  ...
  return {
    title: title,
    description: 'A search in agrofly.ai',
    openGraph: {
      title: title,
      url: `https://agrofly.ai/search/${id}`,
      description: 'A search in agrofly.ai',
      siteName: 'agrofly.ai',
      images: [
        {
          url: `https://agrofly.ai/api/og/chat/${id}`,
          width: 1200,
          height: 630,
        },
      ],
    ...
    alternates: {
      canonical: `https://agrofly.ai/search/${id}`,
    },
  } as Metadata;
}
